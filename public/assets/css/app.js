...
var ReserveView = (function () {
    ...
    ReserveView.prototype.setParam = function (id, key, value) {
        ...
        for (var i = 0; i < len; i++) {
            if (this.data.data[i].id.indexOf(id) != -1) {
                this.data.data[i][key] = value;
                break;
            }
        }
    };
    ...
    ReserveView.prototype.generateItemTemplate = function (data) {
        ...
        template += "<div class='text'>" +
            ...
            "<div>" +
            "<p class='people'>予約人数：" + data.num + "名</p>" +
            "<p class='date'>予約時間：" + this.generateDate(new Date(data.date)) + "</p>";
...
var App = (function () {
    ...
    App.prototype.getMenuList = function () {
        ...
        $.ajax({
            ...
            // url: this.API_URL + "reqUpdate",
            // url: this.API_URL + "reqUpdate.json",
            // url: "https://funclub-server.herokuapp.com/menu_list",
            // url: "//www.ekidata.jp/api/l/11302.json",
            dataType: "json",
            type: "get"
        })
            .then(
